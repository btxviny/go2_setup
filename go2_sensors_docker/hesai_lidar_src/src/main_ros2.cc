...
public:
  ...
  { 
    ...
    lidarPublisher = this->create_publisher<sensor_msgs::msg::PointCloud2>("rslidar_points",1000);
...
private:
  ...
  void node_control_callback(const std_msgs::msg::String::SharedPtr _ptrstring){//lz:关闭节点回调函数
      if(_ptrstring->data == "hesailidar_stop" || _ptrstring->data == "all_stop"){
          rclcpp::shutdown();
          std::cout<<"\033[1;31m"<<"dog_control is forced to stop"<<"\033[0m"<<std::endl;
          exit(0);
      }
  }
  ...
  void lidarCallback(boost::shared_ptr<PPointCloud> cld, double timestamp, hesai_lidar::msg::PandarScan::SharedPtr scan) // the timestamp from first point cloud of cld
  {
    if(m_sPublishType == "both" || m_sPublishType == "points"){
      ...
      //std::cout<<" point size ==================================== "<< static_cast<int>(cld->points.size()) <<std::endl;
