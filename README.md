...
- **RealSense D435i + Hesai PandarXT-16 run as native ROS2 Humble nodes**
  inside a Docker container (`go2-realsense-humble`) on the Go2 expansion
...
Instead, **`go2-realsense-humble`** (built from source on
...
vendored Hesai source) lives on the dock at `~/realsense_humble_docker/`,
mirrored in this repo under [`realsense_humble_docker/`](realsense_humble_docker/)
...
(PC)
```bash
scp -r realsense_humble_docker unitree@192.168.123.18:~/
```

(dock)
```bash
cd ~/realsense_humble_docker
docker build -t go2-realsense-humble .    # 30-90 min first time (librealsense2 compile)
...
starts the container, bridges RealSense + Hesai topics onto an isolated
ROS2 domain (42), and launches RViz2 — one command:
...
Ctrl+C (or closing the RViz2 window) stops everything cleanly (local
`domain_bridge` + `docker compose down` on the dock).
...
(`ssh unitree@192.168.123.18 docker ps`)? Is more than one `domain_bridge`
running locally (`pgrep -af domain_bridge` — a leftover instance from an
earlier session is a real failure mode that's been hit before)? Anything in
`docker logs --tail 50 go2-realsense-humble` on the dock?
...
docker exec go2-realsense-humble chown -R $(id -u):$(id -g) /rosbags/my_bag   # fix root-owned files
```
...
- **No shared TF between `rslidar` and the RealSense's own frame** — each
  renders correctly only when its own frame is the RViz Fixed Frame. Not an
  issue for the current RViz configs (Image displays don't need TF; the
  point cloud display's Fixed Frame is already set to its own frame).
...
realsense_humble_docker/
  Dockerfile, docker-compose.yml, start.sh   — builds/runs go2-realsense-humble (mirrors ~/realsense_humble_docker/ on the dock)
...
tools/
  ...
  cyclone_domain0_enp3s0.xml          — domain_bridge's Cyclone config (dual-scoped: domain 0 on enp3s0, domain 42 on lo)
  cyclone_domain42_lo.xml             — Cyclone config for domain 42 (loopback), used by rviz2/ros2 CLI
  launch_all_sensors_docker.sh        — docker compose up -d + domain_bridge + RViz2, --ethernet (default) or --wifi
  bridge_docker.yaml                  — domain_bridge whitelist (RealSense + Hesai topics)
...
- [`realsense_humble_docker/`](realsense_humble_docker/) — the Docker build context
- [`tools/launch_all_sensors_docker.sh`](tools/launch_all_sensors_docker.sh) — live visualization
- [`realsense_humble_docker/record_sensors_bag.sh`](realsense_humble_docker/record_sensors_bag.sh) — rosbag recording
